 ...
 */
export async function composePost(imageBuffer, story, logoSvgPath = "./assets/logo/tx-mark.svg") {
  ...
  const logoMark = fs.existsSync(logoSvgPath)
    ? fs.readFileSync(logoSvgPath, "utf-8").replace(/<\?xml.*?\?>/, "")
    : "";
...
<svg width="${SIZE}" height="${SIZE}" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- TX corner mark, bottom right -->
  <g transform="translate(${SIZE - 140}, ${SIZE - 140}) scale(0.9)">
    ${logoMark}
  </g>
</svg>`.trim();
